...
public class manager : MonoBehaviour
{
    ...
    public void Update()
    {
        ...
        if (Physics.Raycast(ray, out hit))
        {
            ...
            if (TMS.selected != null && TMS.selected.GetComponent<prop>().getMovementStateEnum(1) == TMS.selected.GetComponent<prop>().unitMoveState)
            {
                //Check to see if the cursor is in range, we cant show movement outside of range so there is no point if its outside
                if (TMS.MovementRange.Contains(TMS.nodes[cursorX, cursorY]))
                {
                    //Generate new path to cursor try to limit this to once per new cursor location or else its too many calculations



                    if (cursorX != TMS.selected.GetComponent<prop>().width || cursorY != TMS.selected.GetComponent<prop>().height)
                    {
                        if (!unitPathExists && TMS.selected.GetComponent<prop>().movementQueue.Count == 0)
                        {
                            ...
                            if (unitPathToCursor.Count != 0)
                            {
                                ...
                                {
                                    int nodeX = unitPathToCursor[i].w;
                                    int nodeY = unitPathToCursor[i].h;
                    ...
                    else if (cursorX == TMS.selected.GetComponent<prop>().width && cursorY == TMS.selected.GetComponent<prop>().height)
                    {
    ...
    public void resetUnitsMovements(GameObject teamToReset)
    {
        ...
        {
            unit.GetComponent<prop>().moveAgain();
        }
    }
    ...
    public void cursorUIUpdate()
    {
        ...
        if (hit.transform.CompareTag("Tile"))
        {
            ...
            {
                selectedXTile = hit.transform.gameObject.GetComponent<click>().width;
                selectedYTile = hit.transform.gameObject.GetComponent<prop>().height;
            ...
            {
                selectedXTile = tileBeingDisplayed.GetComponent<click>().width;
                selectedYTile = tileBeingDisplayed.GetComponent<click>().height;
                TMS.quadOnMapCursor[selectedXTile, selectedYTile].GetComponent<MeshRenderer>().enabled = false;

                selectedXTile = hit.transform.gameObject.GetComponent<click>().width;
                selectedYTile = hit.transform.gameObject.GetComponent<click>().height;
                cursorX = selectedXTile;
                cursorY = selectedYTile;
                TMS.quadOnMapCursor[selectedXTile, selectedYTile].GetComponent<MeshRenderer>().enabled = true;
                tileBeingDisplayed = hit.transform.gameObject;
        ...
        else if (hit.transform.CompareTag("Unit"))
        {
            if (tileBeingDisplayed == null)
            {
                selectedXTile = hit.transform.parent.gameObject.GetComponent<click>().width;
                selectedYTile = hit.transform.parent.gameObject.GetComponent<click>().height;
                cursorX = selectedXTile;
                cursorY = selectedYTile;
                TMS.quadOnMapCursor[selectedXTile, selectedYTile].GetComponent<MeshRenderer>().enabled = true;
                tileBeingDisplayed = hit.transform.parent.gameObject.GetComponent<prop>().Occup;

            }
            else if (tileBeingDisplayed != hit.transform.gameObject)
            {
                if (hit.transform.parent.gameObject.GetComponent<prop>().movementQueue.Count == 0)
                {
                    selectedXTile = tileBeingDisplayed.GetComponent<click>().width;
                    selectedYTile = tileBeingDisplayed.GetComponent<click>().height;
                    TMS.quadOnMapCursor[selectedXTile, selectedYTile].GetComponent<MeshRenderer>().enabled = false;
                    ...
                    cursorY = selectedYTile;
                    TMS.quadOnMapCursor[selectedXTile, selectedYTile].GetComponent<MeshRenderer>().enabled = true;
                    tileBeingDisplayed = hit.transform.parent.GetComponent<prop>().Occup;
    ...
    public void unitUIUpdate()
    {
        if (!displayingUnitInfo)
        {
            if (hit.transform.CompareTag("Unit"))
            {
                ...
                var highlightedUnitScript = hit.transform.parent.gameObject.GetComponent<prop>();
            ...
            else if (hit.transform.CompareTag("Tile"))
            {
                ...
                {
                    unitBeingDisplayed = hit.transform.GetComponent<click>().unitOnTile;

                    UIunitCanvas.enabled = true;
                    displayingUnitInfo = true;
                    var highlightedUnitScript = unitBeingDisplayed.GetComponent<prop>();
        ...
        else if (hit.transform.gameObject.CompareTag("Tile"))
        {
            ...
            else if (hit.transform.GetComponent<click>().unitOnTile != unitBeingDisplayed)
            {
                UIunitCanvas.enabled = false;
                displayingUnitInfo = false;
            }
    ...
    public void teamHealthbarColorUpdate()
    {
        for (int i = 0; i < numberOfTeams; i++)
        {
            ...
            if (team == returnTeam(currentTeam))
            {
                ...
                {
                    unit.GetComponent<prop>().changeHealthBarColour(0);
                }
            }
            else
            {
                foreach (Transform unit in team.transform)
                {
                    unit.GetComponent<prop>().changeHealthBarColour(1);
                }
            }
        }
    ...
    public List<Node> generateCursorRouteTo(int x, int y)
    {
        ...
        if (TMS.selected.GetComponent<prop>().width == x && TMS.selected.GetComponent<prop>().height == y)
        {
        ...
        Node source = TMS.nodes[TMS.selected.GetComponent<prop>().width, TMS.selected.GetComponent <prop>().height];
        Node target = TMS.nodes[x, y];
        dist[source] = 0;
        prev[source] = null;
        //Unchecked nodes
        ...
        while (unvisited.Count > 0)
        {
            ...
            foreach (Node n in u.children)
            {

                //float alt = dist[u] + u.DistanceTo(n);
                float alt = dist[u] + TMS.costToEnterTile(n.w, n.h);
                if (alt < dist[n])
                {
                    dist[n] = alt;
                    prev[n] = u;
    ...
    {
        Vector2 previousTile = new Vector2(unitPathToCursor[i - 1].w + 1, unitPathToCursor[i - 1].h + 1);
        Vector2 currentTile = new Vector2(unitPathToCursor[i].w + 1, unitPathToCursor[i].h + 1);
        Vector2 nextTile = new Vector2(unitPathToCursor[i + 1].w + 1, unitPathToCursor[i + 1].h + 1);
